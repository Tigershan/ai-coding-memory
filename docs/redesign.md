...
- **v1.2**（本版本）：吸收开源方案借鉴。新增 AGENTS.md 双写通道（覆盖不支持 MCP 的 agent，ADR-11）；新增轻量冲突检测（避免"过期规则被召回，比没记忆更危险"，ADR-12）；召回引擎中间台阶改为 SQLite FTS5；reflect/合并机制列入待定（P6 候选）。
...
| **可控性**：用户能改、能删、能恢复 | `source: manual` 永不覆盖 / `archive` 软删 / `.cold/` 冷存储可恢复 |
...
- `should_keep: false` → 写入 `.cold/`，不进召回索引（用户可 `ai-memory restore <id>` 找回）
...
def submit_distill_result(task_id: str, result_yaml: str) -> str:
    ...
    服务端会：
      ...
      - 落盘 should_keep=false 的 topic 到 .cold/
...
def search(query, scope="auto", workspace=None):
    ...
    results = []
    for path in paths:
        # 1. 精确匹配 frontmatter.tags / id：高分
        # 2. 全文 grep：基础分
        # 3. value 加权：high × 1.5, low × 0.5
        # 4. source=manual/edited 加权（人改过 = 高质量）× 1.3
...
3. **跨项目高相关**：其他 projects 中 tags 重合 ≥ 2 或标题 token 相似度高 的条目

实现：第 3 步先用倒排（tags → file），再用 token Jaccard，控制在 < 100ms。

#### 6.6.3 召回引擎演进路径（数据规模触发）

不一步到位上 embedding。按数据规模阶梯演进，每一档都是 stdlib 或零外部依赖：

| 文件数 | 引擎 | 依赖 | 说明 |
|---|---|---|---|
| < 200 | grep（当前 §6.6.1） | 纯 stdlib | 单次召回 < 200ms，扫一遍完事 |
| 200–2000 | **SQLite FTS5** | Python 自带 sqlite3 + FTS5 | mtime 触发增量索引；BM25 排序内置；相同零外部依赖 |
| > 2000 | + embedding 双通道 | 需要 embedding 模型（API 或本地） | 仅 FTS5 召回不够时启用；走 `~/.ai-memory/.embed/` 缓存 |

切换点：当 `ls ~/.ai-memory/{personal,projects}/**/*.md | wc -l` 跨阈值时，CLI `ai-memory rebuild-index` 执行升级。
**关键**：FTS5 这一档让我们能用零依赖支撑到几千条 memory（足够覆盖 95% 用户的 1-2 年用量），不是必须上 embedding 才能扩展。
...
### ADR-8：should_keep=false 不删，进冷存储

**问题**：LLM 一锤定音容易误杀。
**决策**：LLM 判 false 的写 `.cold/`，不进召回索引但保留可 restore。
**理由**：用户事后能审、能恢复，建立信任。
...
**总工时 ~8 天**（Phase 4 + 0.5 天 AGENTS.md 同步；Phase 5 + 0.5 天冲突检测）。
